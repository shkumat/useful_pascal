const   ABOUT0= 'Filter of forbidden characters; ver.1.02 (2023-07-09); author Oleksii Shkumat.'
...
begin
    ...
    for I:=1 to Length(Line) do
        for J:=0 to 31 do
            if Line[I]=Chr(J) then
                if (J<>9) or (not EnableTabChar) then
                    if FixingMode then
                        Line[I]:=#32
                    else    
                        LineIsValid:=FALSE;
...
begin
    for K:=1 to ParamCount do
        begin
            ...
            if (S='/f') or (S='-f') then 
               FixingMode:= TRUE;  
            if (S<>'/t') and (S<>'-t') and (S<>'/v') and (S<>'-v') and (S<>'/n') and (S<>'-n') and (S<>'/f') and (S<>'-f') then
                if InputFileName = '' then
                    InputFileName := S
                else
                    OutputFileName:= S;
